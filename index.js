...
app.get('/student-app', authenticateToken, async (req, res) => {
  try {
    ...
    // 1. Get student's class_id first
    ...
    // 2. Get all appointments for the student with counselor names
    const appointmentResult = await pool.query(
      `SELECT a.*, 
              c.first_name AS counselor_first_name, 
              c.last_name AS counselor_last_name,
    ...
    // 4. Check if student is class mayor
    const mayorResult = await pool.query(
      'SELECT is_class_mayor FROM student WHERE id = $1',
      [studentId]
    );
    ...
    // 6. Get the two latest readings/resources
    const readingsResult = await pool.query(`
      SELECT r.id, r.title, r.description, c.name AS category, r.read_time AS duration
      FROM resources r
      LEFT JOIN categories c ON r.category_id = c.id
    ...
    // 7. Get count of psycho tests for student's class
    let classPsychoTestsCount = 0;
    if (classId) {
      const psychoTestResult = await pool.query(
        `SELECT COUNT(*) AS count 
         FROM psycho_tests 
         WHERE class_id = $1`,
        [classId]
      );
      classPsychoTestsCount = psychoTestResult.rows[0]?.count || 0;
    }
    ...
    res.render('student-app', {
      ...
      recommendedReadings,
      classPsychoTestsCount: parseInt(classPsychoTestsCount)
    });
